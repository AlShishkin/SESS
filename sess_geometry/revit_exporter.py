...
Выбор типов (большое окно):
- Таблица: [✔] Category | Type | Family | TypeParamsJson
- Кнопки: Select All / Clear / Finish
...
- Для дверей/окон: один bbox + ориентированный footprint (ширина×толщина хоста) + параметры
"""
>>>>>>> Stashed changes
...
def _collect_rooms(document, boundary_location, levels_map):
    out=[]
    opts=DB.SpatialElementBoundaryOptions(); opts.SpatialElementBoundaryLocation=boundary_location
    fec=DB.FilteredElementCollector(document).OfCategory(DB.BuiltInCategory.OST_Rooms).WhereElementIsNotElementType()
    for r in fec:
        if not isinstance(r, DB.SpatialElement): continue
        outer,inners=_loops_from_boundary(r.GetBoundarySegments(opts))
        if not outer: continue
        area_ft2=float(getattr(r,"Area",0.0)) if hasattr(r,"Area") else None
        loops={"outer":{"xy_ft":_xy_ft_from_pts(outer),
                        "xy_m": _xy_m_from_pts(outer),
                        "signed_area_ft2": _signed_area_xy_ft(outer)},
               "inners":[]}
        for inn in inners:
            loops["inners"].append({"xy_ft":_xy_ft_from_pts(inn),
...
def _gather_glazing_types():
    ...
    for sdoc, _, _ in _docs_sources(doc):
        for fs in DB.FilteredElementCollector(sdoc).OfClass(DB.FamilySymbol).OfCategory(DB.BuiltInCategory.OST_Windows):
            ...
            if key in seen: continue
            rows.append({"cat":"Windows","type_id":fs.Id.IntegerValue,"type_name":tname,
                         "family": getattr(getattr(fs,'Family',None),'Name',""), "params_json":_flatten_params_simple(_collect_params_block(fs))})
            seen.add(key)
        for fs in DB.FilteredElementCollector(sdoc).OfClass(DB.FamilySymbol).OfCategory(DB.BuiltInCategory.OST_CurtainWallPanels):
            tname = fs.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM)
            tname = tname.AsString() if tname else getattr(fs,'Name','')
        ...
        for wt in DB.FilteredElementCollector(sdoc).OfClass(DB.WallType):
            try:
                if getattr(wt, "Kind", None) == DB.WallKind.Curtain:
                    ...
                    if key in seen: continue
                    rows.append({"cat":"Curtain Walls","type_id":wt.Id.IntegerValue,"type_name":wt.Name,
                                 "family":"WallType", "params_json":_flatten_params_simple(_collect_params_block(wt))})
                    seen.add(key)
            except: pass
    rows.sort(key=lambda r:(r["cat"], r["type_name"]))
    return rows
...
class TypePickerForm(Form):
    def __init__(self, rows, title):
        ...
        col_params = DataGridViewTextBoxColumn(); col_params.HeaderText = "TypeParamsJson"; col_params.Width = 600
...
class MainExportForm(Form):
    def __init__(self):
        ...
        grid = TableLayoutPanel(); grid.Dock = DockStyle.Fill; grid.ColumnCount = 3; grid.RowCount = 4
        ...
        self.cb_rooms = CheckBox(Text="Помещения"); self.cb_rooms.Checked = True
        ...
        self.btn_glaz = Button(Text="Выбрать типы остекления"); self.btn_glaz.Width = 200
        ...
        grid.Controls.Add(self.btn_glaz, 1, 1); grid.Controls.Add(self.lbl_glaz, 2, 1)

        self.btn_doors = Button(Text="Выбрать типы дверей"); self.btn_doors.Width = 200
        ...
        grid.Controls.Add(self.btn_doors, 1, 2); grid.Controls.Add(self.lbl_doors, 2, 2)

        self.btn_export = Button(Text="Экспортировать"); self.btn_export.Width = 180
    ...
    def _do_export(self, sender, args):
        ...
        glazing = {"windows":[], "curtain_panels":[], "curtain_walls":[]} if self.cb_glaz.Checked else {"windows":[], "curtain_panels":[], "curtain_walls":[]}
        doors = [] if self.cb_doors.Checked else []

        if self.cb_glaz.Checked:
            glazing = _export_glazing(self.sel_glazing)
        if self.cb_doors.Checked:
            doors = _export_doors(self.sel_doors)

        out = {
            "version":"bess-export-UI-2",
            "units_note":"Internal length units are feet. Geometry exported as ft and m.",
            "meta": _meta(self.levels_map),
            "levels": levels_list,
            "filters": {
                "rooms": bool(self.cb_rooms.Checked),
                "glazing_types": {k:list(map(int, v)) for k,v in self.sel_glazing.items()},
                "door_types": {k:list(map(int, v)) for k,v in self.sel_doors.items()}
            },
            "rooms": rooms,
            "elements": {"glazing": glazing, "doors": doors}
        }

        path = _pick_save_path()
        if not path:
            TaskDialog.Show("REVIT_DATA_EXPORT", "Path not selected."); return
        ...
        TaskDialog.Show("REVIT_DATA_EXPORT",
                        "Exported:\n{}\nRooms: {}\nGlazing: win:{} pnl:{} cw:{}\nDoors: {}".format(
                            path, len(rooms),
                            len(glazing["windows"]), len(glazing["curtain_panels"]), len(glazing["curtain_walls"]),
                            len(doors)
                        ))
        self.Close()

# ---------------- Export helpers ----------------
def _export_glazing(sel_glazing):
    out = {"windows":[], "curtain_panels":[], "curtain_walls":[]}
    ...
    for sdoc, linkinst, T in _docs_sources(doc):
        # Windows with oriented footprint
        if windows_ids:
            col = DB.FilteredElementCollector(sdoc).OfCategory(DB.BuiltInCategory.OST_Windows).WhereElementIsNotElementType()
            for el in col:
                try:
                    if not isinstance(el, DB.FamilyInstance): continue
                    sym = el.Symbol
                    if not sym or sym.Id.IntegerValue not in windows_ids: continue
                    bbox = _bbox_record(el, T)
                    if not bbox: continue
                    center = _bb_center(el, T)
                    ...
                    width = height = None
                    try:
                        p = el.get_Parameter(DB.BuiltInParameter.WINDOW_WIDTH)
                        if p and p.StorageType==DB.StorageType.Double: width = p.AsDouble()
                    except: pass
                    try:
                        p = el.get_Parameter(DB.BuiltInParameter.WINDOW_HEIGHT)
                        if p and p.StorageType==DB.StorageType.Double: height = p.AsDouble()
                    except: pass
                    ...
                    fp_ring = _rect_footprint(center, hand, facing, width, depth) if (center and hand and facing and width and depth) else None
                    footprint = _footprint_record(fp_ring)

                    rec = {"kind":"window","id": el.Id.IntegerValue, "unique_id": getattr(el,"UniqueId",None),
                           "type_id": sym.Id.IntegerValue, "type_name": _element_typename(el, sdoc),
                           "bbox": bbox, "footprint": footprint, "params": _collect_all_params(el, sdoc)}
                    if facing: rec["params"].setdefault("facing_dir",[round(facing.X,6),round(facing.Y,6),round(facing.Z,6)])
                    if hand:   rec["params"].setdefault("hand_dir",[round(hand.X,6),round(hand.Y,6),round(hand.Z,6)])
                    if host_thk is not None: rec["params"].setdefault("host_thickness_ft", round(host_thk,6))
                    if linkinst is not None:
        ...
        if panels_ids:
            ...
            for el in col:
                try:
                    ...
                    if not fi: continue
                    sym = fi.Symbol
                    if not sym or sym.Id.IntegerValue not in panels_ids: continue
                    bbox = _bbox_record(fi, T)
                    if not bbox: continue
                    rec = {"kind":"curtain_panel","id": fi.Id.IntegerValue, "unique_id": getattr(fi,"UniqueId",None),
                           "type_id": sym.Id.IntegerValue, "type_name": _element_typename(fi, sdoc),
                           "bbox": bbox, "params": _collect_all_params(fi, sdoc)}
                    if linkinst is not None:
                        rec["in_link"]={"link_instance_id":linkinst.Id.IntegerValue,
                                        "link_doc_title":sdoc.Title,
                                        "link_doc_full_path":_user_model_path(sdoc)}
        ...
        if cw_ids:
            ...
            for el in col:
                try:
                    ...
                    rec = {"kind":"curtain_wall","id": el.Id.IntegerValue, "unique_id": getattr(el,"UniqueId",None),
                           "type_id": wt.Id.IntegerValue, "type_name": wt.Name,
                           "bbox": bbox, "params": _collect_all_params(el, sdoc)}
                    if linkinst is not None:
                        rec["in_link"]={"link_instance_id":linkinst.Id.IntegerValue,
                                        "link_doc_title":sdoc.Title,
                                        "link_doc_full_path":_user_model_path(sdoc)}
...
def _export_doors(sel_doors):
    door_type_ids = set()
    for k,v in sel_doors.items():
        if k=="Doors": door_type_ids |= set(v)
    res=[]
    if not door_type_ids: return res
    for sdoc, linkinst, T in _docs_sources(doc):
        col = DB.FilteredElementCollector(sdoc).OfCategory(DB.BuiltInCategory.OST_Doors).WhereElementIsNotElementType()
        for el in col:
            try:
                if not isinstance(el, DB.FamilyInstance): continue
                sym = el.Symbol
                if not sym or sym.Id.IntegerValue not in door_type_ids: continue
                bbox = _bbox_record(el, T)
                if not bbox: continue
                ...
                width = height = None
                try:
                    p = el.get_Parameter(DB.BuiltInParameter.DOOR_WIDTH)
                    if p and p.StorageType==DB.StorageType.Double: width = p.AsDouble()
                except: pass
                try:
                    p = el.get_Parameter(DB.BuiltInParameter.DOOR_HEIGHT)
                    if p and p.StorageType==DB.StorageType.Double: height = p.AsDouble()
                except: pass
                if width is None or height is None:
                    sz = _bb_size(el, T)
                ...
                rec = {"id":el.Id.IntegerValue, "unique_id":getattr(el,"UniqueId",None),
                       "type_id": sym.Id.IntegerValue, "type_name": _element_typename(el, sdoc),
                       "bbox": bbox, "footprint": footprint, "params": _collect_all_params(el, sdoc)}
                if facing: rec["params"].setdefault("facing_dir",[round(facing.X,6),round(facing.Y,6),round(facing.Z,6)])
                if hand:   rec["params"].setdefault("hand_dir",[round(hand.X,6),round(hand.Y,6),round(hand.Z,6)])
